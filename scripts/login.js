...
function storeInitialUsers() {
    const users = [
        { username: 'ahmad@gmail.com', password: 'aaz@1999' },
        { username: 'john@example.com', password: 'john1234' }
    ];
...
username.addEventListener('input', function (e) {
    ...
    if (valid) {
        ...
        document.getElementById('username-err').textContent = 'Invalid email format.';
    }
});
...
password.addEventListener('input', function (e) {
    ...
    if (valid) {
        ...
        document.getElementById('password-err').textContent = 'Password must be at least 8 characters long.';
    }
});
...
function storFormData(event) {
    ...
    if (!passwordPattern.test(enteredPassword)) {
        passwordErrMessage.textContent = 'Password must be at least 8 characters long.';
        return; // Stop further processing
    } else {
